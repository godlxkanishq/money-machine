...
const ERRORS: Record<string, string> = {
  invalid_key: "That license key wasn’t recognized.",
  ...
  default: "Couldn’t activate. Please try again.",
};
      ...
      >
        <div className="mb-6 flex flex-col items-center text-center">
          <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-[14px] bg-accent text-accent-foreground">
            <svg width="24" height="24" viewBox="0 0 32 32" fill="none">
              <path d="M9 22V10.5L16 17l7-6.5V22" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </div>
          <h1 className="text-xl font-semibold tracking-tight">Activate Money Machine</h1>
          <p className="mt-1.5 text-sm text-muted-foreground">Enter your license key to unlock this device.</p>
        </div>
        ...
        <div className="rounded-lg border border-border bg-card p-5 shadow-elevated">
          <form onSubmit={submit} className="space-y-3">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                className="font-mono uppercase tracking-wide placeholder:normal-case"
                spellCheck={false}
              />
            </div>
            ...
            <Button type="submit" variant="primary" size="lg" disabled={busy || !key.trim()} className="w-full">
              {busy ? (
          ...
          <div className="mt-4 flex items-center gap-2 border-t border-border pt-4 text-xs text-muted-foreground">
            ...
            <span>Verified server-side · bound to this device ({shortAddress(deviceId, 6, 4)})</span>
          </div>
        </div>
